...
class Jar:
    ...
    def remove_cookie(self,brand): 
        for all_cookies in self.all_cookies:
            if all([all_cookies.brand == brand]):
                self.all_cookies.remove(all_cookies)
                return True
        return False
    def get_cookie_details(self):
        msg = f'This jar has {len(self.all_cookies)} cookies with {1200} calories\n'
        for all_cookies in self.all_cookies:
            msg += f'{all_cookies.brand} makes a ({all_cookies.flavour}) flavoured cookie which is {all_cookies.size} cm radius and has {all_cookies.calories}\n'
        return msg